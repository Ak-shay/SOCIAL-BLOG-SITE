...
class FollowPostListView(LoginRequiredMixin, ListView):
	"""Returns posts followed by the user. """
	context_object_name = 'posts'
	template_name = 'Blog/post_list.html'

	def get_queryset(self):
		# post of following person 
		following = self.request.user.follow.all()

		if 'search' in self.request.GET:
			searched_value = self.request.GET.get('search')
			posts = Post.objects.filter(title__icontains = searched_value,
										published_date__lte=timezone.now(),
										author_post__in=following)
		else:
			posts = Post.objects.filter(published_date__lte=timezone.now(),
										author_post__in=following).order_by('-published_date')
